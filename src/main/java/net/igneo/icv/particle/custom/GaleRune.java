...
public class GaleRune extends TextureSheetParticle {

    int startRot = 360;
    int renderTicks = 0;
    GaleManager manager;
    Player owner;
    protected GaleRune(ClientLevel pLevel, double pX, double pY, double pZ, double pXSpeed, SpriteSet spriteSet, double pYSpeed, double pZSpeed) {
        ...
        for (Player player : Minecraft.getInstance().level.players()) {
            ...
            if (dist < 1.5) {
                owner = player;
                manager = (GaleManager) ICVUtils.getManagerForType(player, GaleManager.class);
                if (manager != null) {
        ...
        if (users.keySet().size() > 1) {
            ...
            owner = player.getKey();
            manager = (GaleManager) ICVUtils.getManagerForType(player.getKey(), GaleManager.class);
        }
    }
    ...
    public void tick() {
        if (age < 11) {
            for (Vec3 pos: ParticleShapes.renderRotatedRingList(owner.position().add(0,1,0).subtract(x, y, z).normalize(),this.getPos(),3.5f,8,true)) {
                LodestoneParticles.galeParticles(level,pos,owner.position().add(0,1,0).subtract(x, y, z).normalize().scale(3));
            }
        }
        super.tick();
    }
    ...
    public void render(VertexConsumer pBuffer, Camera pRenderInfo, float pPartialTicks) {
        ...
        Player player = owner;
        Vec3 toPlayer = player.position().add(0,1,0).subtract(x, y, z);
        ...
        float roll = (float) renderTicks /10; // or your animation curve
...
// convert to quaternion
    ...
    public static class Provider implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(SimpleParticleType particleType, ClientLevel level,
            ...
            return new GaleRune(level, x, y, z, dx, this.sprites, dy, dz);
        }
    }
}
